...
**泡泡是可以戳的。** 它不是「一块游戏区」——专门辟一块地方放游戏，等于告诉访客
「这儿有个跟你无关的东西」，多数人绕着走。它就是背景本身：路过顺手戳一下有反馈，
不戳也不碍事，不占版面、不抢点击、不需要说明书。指针靠近时泡泡描一圈，这是唯一的
「可以戳」提示。canvas 是 `pointer-events:none` 压在最底下，点击在 document 上接，
落在卡片 / 链接 / 按钮 / 输入框上的一律放行 —— 连「点空白处收起输入焦点」这种
下意识动作都不会被抢走。最多 16 个泡泡 + 60 个碎片，标签页切走就停 rAF。

**`prefers-reduced-motion` 下这一整套都不启动**，内容一个不少。
